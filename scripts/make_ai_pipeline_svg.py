...
def generate_ai_pipeline_svg(output_path="ai-pipeline.svg"):
    ...
    svg_lines.append('    .flow-line { stroke: #58a6ff; stroke-width: 2px; stroke-dasharray: 4 4; }')
    svg_lines.append('    .pulse-dot { fill: #39d353; opacity: 0.8; }')
    svg_lines.append('    .footer { font-family: ui-monospace, SFMono-Regular, "SF Mono", Menlo, Consolas, monospace; font-size: 9.5px; fill: #8b949e; }')
    svg_lines.append('  </style>')
    ...
    for n in nodes:
        ...
        svg_lines.append(f'  <text class="node-sub" x="{n["x"] + 12}" y="{n["y"] + 56}">• {n["sub1"]}</text>')
        svg_lines.append(f'  <text class="node-sub" x="{n["x"] + 12}" y="{n["y"] + 78}">• {n["sub2"]}</text>')
        svg_lines.append(f'  <text class="node-sub" x="{n["x"] + 12}" y="{n["y"] + 100}">• {n["sub3"]}</text>')
    ...
    for i in range(len(nodes) - 1):
        ...
        svg_lines.append(f'  <line x1="{x1}" y1="{y1}" x2="{x2}" y2="{y2}" class="flow-line" />')
        svg_lines.append(f'  <circle cx="{(x1 + x2)/2}" cy="{y1}" r="4" class="pulse-dot" />')

    # Footer status
    svg_lines.append(f'  <text class="footer" x="20" y="{height - 18}">Latency: &lt;150ms/frame | Real-Time Inference | End-to-End PyTorch &amp; FastAPI Pipeline</text>')
